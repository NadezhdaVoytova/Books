...
При возрастании сложности системы возникает необходимость управления и восприятия большими моделелями
* необходимо добиться модульности, которая не потеряет преимуществ интеграции (отдельные части работают совместно, способчтвуя координации операций)
...
* если нет логической согласованности, то модель не имеет смысла. В идельном случае любое логическое утверждение о предметной области было бы непротиворечивым
* поддрежание такой унифакации требует больше усилий, чем дает пользу. Полная унифакация невозможна или затратна и, как следствие, нельзя все потребности удовлетворить одной моделью

Необходимо использовать способ, обозначающий границы и взаимоотношения между объектами. Для этого выделим:
* **ограниченый контекст** - диапазон применимости
...
**Характеристики**
* **Ограниченный контекст - не модуль!** Модули не всегда пытаются ограничить контексты, они устпользуются для организации модели. 
* ограниченный контекстставит пределы применимости какой либо модели, чтобы разработчики поддерживали единообразие и согласованность и понимали в чем она состоит. 
...
## Рaспознавание деффектов внутри ограниченного контекста
**Симптомы**
* несоответсвие запрограммированных интерфейсов (неожиданное поведение от метода)
* путаница в языке
...
Определив ограниченный контекст, следует поддерживать его в хорошем состоянии. При работе в одном контексте большого числа разработчиков, он фрагментируется и распадается на части. Есои разбивать контекст на части, что в нем теряется уровень интеграции и связности

**Непрерывная интеграция (CONTINUOUS INTEGRATION)** состоит в том, что вся работа в пределах контекста сливается воедино и приводится в согласованный вид достаточно часто, чтобы при появлении деффектов, они обнаруживались достаточно быстро
...
Естественный ход организации команды - по ограниченным контекстам (раздаляют ценность контекста модели), подсистемам. 

**Рекомендации**
* опредлить все модели в контексте
* задачать для каждой модели ограниченный контекст
...
* описать точки соприкоснования между моделями, явно задавая трансляцию для любого способа коммуникации и выделяя любые совместные ресурсы
* постройте карту существующей территории в любом представлении

**Шаги при обнаружении смысловых деффектов**
* завершить карту
* вернуться к листам противоречий, имея точный общий вид
* исправив небольшой деффект, ввести специальные процедуры для их целостности
* если не ясна связь, то выбрать близкий по смыслц шаблон и двигаться по нему
...
* выбрать подмножество, над которым группы согласны работать вместе (часть базы-архитектуры). Эти элементы могутизменяться при согласии другой стороны
* выполнять регулярно непрерывную интергацию
...
**Цель** - уменьшить дублирование работыи сделать интеграцию проще
...
* должен существовать набор автотестов, который позволит верхнему уровню вносить изменения без ущерба для нижнего

### Конформист
Как заказчик-поставщик, только без единого руководства. связь "вверх" - "вниз"
* верх главный,может неделиться ресурсами, не сообщает о планах
...
* не всегла удобно и хорошо переписать, надо пробовать интегрироваться

**Проблемы**
* инфраструктурный уровень должен уметь работать с другими протоколами
* нужно работать с другой моделью предметной области
* можно неправильно интерпретировать даныне другой системы
...
* адаптер преобразует данные в семантически верные и отдресует адаптанту
...
* если нужна коммуникация между двуми подсистемами. если к другой системе нет доступа, то средства коммуникации можно поместить между фасадным объектом и другой подсистемой. если фасадный объект можно интегрировать с другой системой напрямую, то коммуникационное звено можно поместить между адаптером и фасадным объектом.
...
* если другая система проста или имеет четкий интерфейм, то фасадный объект может не понадобиться
* в предохранительный уровень можно добавить  логгирование, некоторые рабочие функции
* иногда этот уровен может быть внутри своей системы, если в основе подсистем разные модели
...
## Служюы с открытым протоколом
Чаще всего в любом ограниченном контексте определяется по одному трансляционному уровню для каждого компонента за пределами контекста, с которым необходимо интегрироваться. Если интеграция на один раз, то подход с добавлением одного трансляционного уровня для каждой внешней системы позволяетс предохранить модель с минимальными излишками. Если будет обращаться часто, то кужен гибкий подход

**Рекомендации**
* определить протокол, предоставлящий доступ к системе, как к набору служб
* сделать протокол открытым
* расширить протокол дляновых требований
...
## Унификация модели
Вероятнее всего никто не захочет отказываться от своей модели. Можно попробовать ввести общую абстракция. Если знания со всех сторон сбросить в кучу, то стройной модели не получится, но если знания переработать, то может получится полная унификация модели. **Противорение может быть источником новых моделей**

## Выбор стратегии построения контекстов
Разбор взаимосвязей перед картой контекстов

### Уровень принятий решений или выше
Идеально, если решение принимается всей группой, но в любом случае решение должно быть донесено до каждого члена коменды. НУжно принимать решение о разделении или расширении взвесив преимущества независимой работы против тесной интеграции, найдя компромис
...
* небольшие модели могут обслуживать специальные потребности, соответствующие узной подобласти
...
* если разработчиков больше десяти, то возможно имеет смысл подумать об общем ядре и вынести относительно независимые наборы функций в независимые ограниченные контексты. Если отношения между какими-либо контекстами в одну сторону, то определить отношения заказчик-поставщик
    * если есть конфликты между граппами - можно двум контекстам вести отдельное существование. Если между этими контекстами необходима интеграция, то вывести трансляционный уровень, который сопровождается совместно

### Учет обсобых случаев отдельными моделями
Еслие сть внутренние жаргонизма, то они наносят вред общему языку и не должны существовать в рамках непрерывной интеграции. Самый большой риск - это внесение шума узкими моделями. Главный вопрос: так ли нужно это понятие? велика ли цена трансляции?

### Установка системы
Координация установки изменений одного контекста при неизменном другом. должен сохраняться трансляционный уровень

### Компромисы
Поиск компромисов межуд преимуществами интеграции в одно целое и минусами дополнительной координации

### Если проект в работе
Если необходимо влиться в работу над уже разрабатываемым проектом
* определить контексты
* построить практику вокруг текущней организации
* улучшать непрерывную интеграция вокруг контексто
...
* выбрать одну модель и сделать рефакторинг другого контекста для приведения к совметному виду, можно взять еще связные модели, чтобы не потерять взаимосвязи, можно новую более глубокую модель
...
#### От общего ядра к непрерывной интеграции (полная унифаикация двух контекстов)
* разделить процедуры для непрерывной интеграции между группами
* начать циркуляцию разработчкиов между группами для самостоятельного формирования группы
* объявить **дистиляцию** каждой модели
...
**Слияние моделей смслового ядра**
...
* определить дпоплнения, котоыре понадобятся на пердохранительном уровне
* реализовать
* установить
...
* опеределить существует ли единый язык
...
# Дистиляция
**Дистиляция** - это процесс разделения компонентов смеси с целью выделения основного вещества в той форме, которая делает его более ценным и полезным.
Построение модели - дистиляция моделей
...
* помогает определиться с суб. подрядами, использование готовых компонентов, распрделением задач
...
* в процессе планировани следует направить ресурся на самык сложные и запутанные участки архитектуры

**Рекомендации**
* уменьшить модельь до минимума
* найти смысловое ядро и сделать так, чтобы его можно было отличить от массы вспомогательных частей модели и кода
* выделите самые ценные и специализированыые понятия
* ядро должно быть небольшим
* отправьте на разработку ядра лучшие карты
* вкладывайте ресурсы в другие части системы по уровню важности для поддержки дистилированного смыслового ядра

## Эскалация дистиляции
...
**Рекомендации**
* опеределить, какие мотивы не являются основным мотивом к написанию приложения
...
**Варинты использования подобластей**
* купить готовый модуль
    плюсы: Меньше писать, сопровождение на стороне, возможно, кодж более надежен
    минусы: надо все равно изучить тему,нужен аудит, перегруженность кода, возможные проблемы интеграции
* общедоступная архитекутура или модель
    плюсы: более проработана, есть документации
    минусы: перегруженность линими элементами, несоответствие потребностям
...
Нет необходимости вмегда строить переносимые архитектуры (повторно используемые), но следует строго придерживаться общей концепции
...
Введение в продметную область фокусируется на общих характеристиках модели предметной области
...
* показать, как модель служит разным интересам и сосздает баланс между ними
...
### Дистиляционный документ
Документ со списком существенных концептульаных объектов и схемой их взаимного действия. Это минимальынй набор, очерчивающий ядро
...
### Дистиляционный документ как методическое средство
**Изменение дистиляционного документа** - руководство к действию, индикатор значимости изменений в модели. Если изменения в модели или в коде влияют на дистиляционный документ, то это требует согласовании членов группы. Должен быть в актуальном состоянии у всех. Изменения за пределом ядра может вноситься без согласования, члены группы ознакомятся с изменениями в рабочем порядке

### Связные механизмы
Сложные механизмы работы должны быть инкапсулированы. Если в модели много алгоримтмически(логически) сложных функций, значит, надо искать модель, упрощающую вычислительные механизмы. Нужно выделить **связную часть** механизма. Мехнизмы имеют вспомогательную функцию.
...
### Сравнение связных механизмов и специалмзированной подобласти
...
### Дистиляция к декларативныому стилю
Связыне механизмы полезны, когда предоставляют доступ через информативный интерфейс с утверждениями, концептуально связанными и функциями без побочных эффектов, и, как следствие, оперирование осмысленными декларациями. Выделение специализированных областей делает код менее запутанным, связыне механизмы инкапсулируют сложность, и, как следствие, позволяют сфокусироваться на моедли без примесей.
...
* определить вспомогательные элементы (даже неотнозначно определенные)
...
* все что останется постепенно нужно выделать в специализированные подобласти, но на ранних этапах моожно простовыделить в пакет
...
* рефакторинг с целью упрощения взаимосвязей и взаимодействией в ядре, для минимизации и прояснения связи с другими модулями
...
Решение о выделении ядра принимается только коллективом(самоорганизованым, скоординированным, дисциплинированным). Знания должны распределяться в группе. Архитектура должна быть гибкой для корректирования курса действий.
...
## Абстрактное ядро
Если между подобластями в отдельных модулях происходит интенсивное взаимодейтствие, то либо создается много ссылок между ними, либо организуется косвенное взаимодействие, что ухудшает наглядность модели

Нужно подумать над возможностью установить горизонтальные границы. ПОлиморфизм дает право игнорировать детали различий между экземплярами абстрактного типа. Если взаимодействие между модуялми можно выразить в виде полиморфного интерфейса, то имеет смысл их выделить в отдельный модуль ядра. Полиморфизм ценен, когда соответствует понятиям предметной области (фундаментальным). Отделение абстракци позволяет взаимную зависмость модулейи дистилировать связное смысловое ядро

**Рекомендации**
* определить фундаментальные понятия
* выделить через рефакторинг абстрактные классы или интерфейсы
* спроектировать модель так, чтобы выражала большинство взаимодейтвий между компонентами
* поместить модель в собственный модуль
* конкретные реализации остаются в собственных подобластях
* абстрактное ядро похоже на дистиляционный документ, но в коде

## Дистиляция в углубленных моделях
Дистиляция - не примитичное отделение части предметной области от смыслового ядра. Подобласти совершенствуются, идет движение в сторону углубленной модели и гибкой архитектуры. Углубленная модель сама дистилирует существеные аспекты предметной области в простые элементы, что вместе дает решать практические задачи, стоящие перед приложением
...
Организация уровеней, описывающих разные аспекты телекоммуникационной системы
  ...
  * и т.д каждый уровень отражает свою зону отвественности, новсе вместе отражают рабочий процесс в системе
  * после проработки (когда система станет многоуровневой) уровени будут преставлять схему правил, взаимоотношений, границ определенного модуля или объекта в целостной архитектуре.
  * это позволяет проще искать ту или иную функци, понимать общую структуру системы
 
**2. разработать схему правил или ролей, отношений, распространяемы на всю систему и позволяющую менять место каждой в едином целом без глубоких знаний** 
...
В системе, лишенной общего руководящего принципа, который позволяет воспринимать элементыс точки зрения ролей в стуктурах, разработчики не видят леса за деревьями

**Крупномасштабная структура** - это язык, на котором можно обсуждать, описывать систему крутпными мазками
...
**5. нужно искать компромис между унификацией структуры и свободой выражения**
...
**Структуре нужно следоватьв сегда, за исключений особых ситуаций(исключений). Если исключений много - обдумать структуру повторно или вообще отбросить**

## Типы крупномасштабных архитектур
### Метафорический образ системы
**Образ системы** - нестрогая ясно понятная крупномасштабная архитектура, находящаяся в соответствии с объектной парадигмой. Образ системы - это аналогия предметной области и призительно соответствует рабочей области. Этот образ можно применить в разных ограниченных контекстах, способствуя координированию работ над ними

Чем убедительнее образ, тем больше риск, что архитектура воспримет такие астпекты этой аналогии, которые не желательны при решении конкретной задачи, а какие-то отбросит, и аналогия откажется неуспешной
...
В некотрых областях имеется расслоение. Понятия существуют в связи с другими, некоторые отдельно, у понятий разная скорость изменения
...
* в моделях, где есть естественное расслоение можно построить уровни вокруг основных групп принципов: многоуровневая архитектура, проектирвоание по принципу отвественности
...
**Многоуровневая структура, подходящая для уровней разделения обязанностей, называется нестрогой многоуровневой системой. В ней компонентам того или иного уровня разрешается обращаться к любым нижним уровням, а не только к тому, котоырй расположен сразу под ними**

**Рекомендации**
* изучить связи модели(концептуальные), скорость изменений в разных частях ПО
* если есть естественное расслоение - предствить в виде групп обязанностей
...
* наличик контуров концептуальные: если объекты разных уровней меняются с разной скоростью или по разным причинам, уровни должны отражать наличие сдвигов между ними

**Пример деления**
* потенциал (что можно сделать? с помощью чего?)
* отперации (что происходит? что можно извлечь из потенциала?) - связь наверх
...
* регламент (сложные бизнес-требования)
    **Концептуальные взаимосвязи и точки относительного сдвига уровней в системе автоматизации производства**
    | принятие решений| aналитические механизмы | практически отсутствуют как состояние, так и изменение| анализ управления, оптимизация использования, сокращение рабочего цикла|
    |---------------|--------|------|-----|
...
* обязанности (какие мы дали обещания и кому?) Этот уровень по характеру похож на регламентный в том, что он задаает цели для будущих операций, но и на операционный тоже, поскольку обязательства возникают и изменяются в ходе текущей деятельности

### Уровень знаний
**Уровень знаний** - это группа объектов, которая расписывает, как должна вести себя другая группа объектов
Уровень знаний решает задачу того, чтобы часть модели была поддатливой в руках пользователя и при этом подчинялась широкому набору правил: изменение отношение между сущностями при установке или ходе работы
...
* при реализации уровня предметной области не нужно пользоваться рефлексией, реализованной в языке. Эти мета-объкты описывают структуру и функции самих языковых конструкций, а уровень знаний должен строиться из обыкновенных объектов
...
* для конфигурирвания пользователем нужны знания и навыки
* при изменении архитектуры знаний именогда нужно менять операционный уровень

### Среда подключаемых компонентов
Этот подход актуален, когда есть несколько приложений. Когда необходимо взаимодействие между несколькими ограниченными контекстами, как решение, разбивают архитектуру на функциональные компонеты и подключают к центральному распределительному модулю.
...
**Рекомендация**
* дистилировать абстрактное ядро интерфейсов и связей
...
* структура идеально подходит для разных источников или инкапсулируетранее написанное с целью интегрирования
...
* объекты операционного уровня генерируют события, а регламентый прослушивает (верхний генерирует, нижний слушает). Если событие нарушает регламентное правило, то выполняется ответное действие по правилу, либо генерирует событие более высокого уровня
...
* минимализм: решение насущных и серьезных задач. Выбираем нежесткую структуру, похожую на образ системы, и разделение обязанностей
* коммуникативность и самодисциплина: говорить, обсуждать, чтобы не разлагаться
* реструктуризация дает гибкую архитектуру: перход от образа к уровням. вызвано необходимостью развития, получение новых знаниц
* непрерывная дистиляция: например, определение неспецифической подобласти в системе разделения уровней. В подключаемых компонентах может стать общим ядром. Уровни опеределяются точнее по менре получения знаний

# Определение стратегических подходов
## Сочетание крупномасштабных структур и ограниченных контекстов
**Основные принципы**: компактность, дистиляция, крупномасштабная структура
...
* распределить контексты на уровни, а значит, интеграции считаются расположенными на всех уронях, а фасады взаимодействия можно вывести на один уровень

Внутри одного контекста может быть своя структура, внутри другого своя, а всю карту организовать в соотвествии с третьей

## Сочетание крупномасштабной структуры и дистиляции
* крупномасштабная структура помогвет во взаимоотношениях смыслового ядра и неспециализированных подобластей
* помогает дистилировать смысловое ядро через выделение уровней операций, регламентов
...
* есть ли смысловое ядро? напсиано ли введение в предметную область? Можно ли написать?
...
* человек имеющий большие полномочия. Особенно, если он практикующий разработчие (неформальный лидер)
...
* решения должны довдиться до всех членов группы. Необходим высокий урвоень коммуникации. Если его нет, разработчики бкдкт игнорировать решения архитекторов
...
* план должен допускать эфолюцию и развитие: не должно быть препятствий для развития преокта. Если план накладывает много ограничений, нужен ли он. О проблемах нужно узнавать от разработчиков
* разработчки архитектуры не должны переманивать лучшие кадры. Если у разработчиков низкие навыки проектирования, то при идеальной архитектуребкдет плохая реализация. Также у архитекторов должны быть технически осведомленные разработчики. Необходим баланс.
* в стратегическом проектирвоании нужен минимализм и скромность. Необходимо сформировать организующие принципы и ключевые модел, очищенные от всего, что не помогает улучшить четкость и ясность архитектуры
...
* долой генеральный план: если он слишком жесток или мягок, тогда в нем нет смысла. На его место ставим ограниченный порядок, приспособленный к реальным обстоятельствам: набор принципов, действующих в отношении всех членой сообщества и применяемый в любой мелкой задаче.
